...
#include "mymediapipe/calculators/phi/controller/ani_controller.h"
...
public:
    PhiAniEglCalculator() : initialized_(false), count_(0) {}
    PhiAniEglCalculator(const PhiAniEglCalculator&) = delete;
    PhiAniEglCalculator& operator=(const PhiAniEglCalculator&) = delete;
    ~PhiAniEglCalculator();
...
REGISTER_CALCULATOR(PhiAniEglCalculator);
...
// static
absl::Status PhiAniEglCalculator::GetContract(CalculatorContract* cc) {
...
absl::Status PhiAniEglCalculator::Open(CalculatorContext* cc) {
...
absl::Status PhiAniEglCalculator::Process(CalculatorContext* cc) {
...
absl::Status PhiAniEglCalculator::Close(CalculatorContext* cc) {
...
PhiAniEglCalculator::~PhiAniEglCalculator() {
  RunInGlContext([this] {
    framebuffer_target_.reset();
  });
}

absl::Status PhiAniEglCalculator::GlSetup() {
...
absl::Status PhiAniEglCalculator::GlBind() {
    // Setup some OpenGL options
    return absl::OkStatus();
}

absl::Status PhiAniEglCalculator::GlRender(CalculatorContext* cc, 
                                            const GlTexture& src, 
                                            const GlTexture& dst,
                                            double timestamp) {
...
absl::Status PhiAniEglCalculator::GlCleanup() {
    // cleanup
    glFlush();
    return absl::OkStatus();    
}

absl::Status PhiAniEglCalculator::GlTeardown() {
    // ourShader->tearDown();
    return absl::OkStatus();
}
