#include "multi_chain.h"

MultiChain::MultiChain(std::vector<Chain*> chains) {
    root = new ChainNode();
    root->value = *chains.begin();
    root->parent = NULL;
    root->children = new std::vector<ChainNode*>();
    bool success = false;
    ...
    if (!success) {
        std::cerr << "Something went wrong when constructing the multichain; one of the nodes probably didn't match up." << std::endl;
        exit(-1);
    }
}
...
bool MultiChain::Insert(ChainNode * root, Chain * chain) {
    if (glm::all(glm::equal(root->value->end, chain->origin))) {
        ...
        new_node->children = new std::vector<ChainNode*>();
